...
export class AppUserList implements OnInit {
  ...
  private static readonly FILTERS_KEY = 'app-user-list-filters';
  private static readonly SORT_KEY = 'app-user-list-sort';
  private static readonly PAGE_KEY = 'app-user-list-page';

  readonly users = signal<AppUser[]>([]);
  readonly loading = signal(false);
  readonly filterVisible = signal(false);

  /** Tri-state 啟用 filter options for the drawer p-select. */
  ...
  filter: AppUserQuery = { keyword: null, isActive: null };
  ...
  private restoreState(): void {
    const filters = this.readJson<AppUserQuery>(AppUserList.FILTERS_KEY);
    if (filters) this.filter = { ...this.filter, ...filters };

    const sort = this.readJson<ListSort>(AppUserList.SORT_KEY);
    if (sort) {
      this.sortField = sort.sortField;
      this.sortOrder = sort.sortOrder;
    }

    const page = this.readJson<ListPage>(AppUserList.PAGE_KEY);
    if (page) {
  ...
  private persistFilters(): void {
    sessionStorage.setItem(AppUserList.FILTERS_KEY, JSON.stringify(this.filter));
  }

  private persistSort(): void {
    const sort: ListSort = { sortField: this.sortField, sortOrder: this.sortOrder };
    sessionStorage.setItem(AppUserList.SORT_KEY, JSON.stringify(sort));
  }

  private persistPage(): void {
    const page: ListPage = { first: this.first, rows: this.rows };
    sessionStorage.setItem(AppUserList.PAGE_KEY, JSON.stringify(page));
  }
